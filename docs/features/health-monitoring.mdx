...
Health Monitoring is VMLedger's continuous surveillance system that watches your VMs 24/7. It combines two powerful monitoring techniques:
1. **Health Checks** - Quick connectivity tests every 60 seconds
2. **Metrics Collection** - Detailed resource usage every 5 minutes

<Info>
**Real-World Analogy**: Think of health monitoring like a hospital's patient monitoring system. The heart rate monitor (health checks) beeps every second to confirm the patient is alive, while the nurse (metrics collection) comes by every few minutes to check temperature, blood pressure, and other vitals.
</Info>
...
<CardGroup cols={2}>
  <Card title="Health Checks" icon="clock">
    ...
    **Configurable:** Yes (via `PING_INTERVAL` env var)
...
<CardGroup cols={2}>
  <Card title="Monitor Critical VMs More Frequently" icon="gauge-high">
    ...
    METRICS_INTERVAL=180  # Every 3 minutes
    ```
  </Card>
  ...
  <Card title="Use Longer Intervals for Many VMs" icon="gauge-low">
    ...
    ```bash
    PING_INTERVAL=120  # Every 2 minutes
    METRICS_INTERVAL=600  # Every 10 minutes
    ```
  </Card>
